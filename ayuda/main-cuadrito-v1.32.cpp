//C�digo de proeycto Cuadrito del grupo 04:
...
const int MJug = 5; //Constante para el tama�o de puntos y gamertag
...
void actualizar(int fila, int col, char tablero[][M_columnas]);  //mostrar el tablero tras actualizaci�n de movimiento de un jugador
void tu_turno(int fila, int col,int &turno,char tablero[][M_columnas],jugador partida[],int puntos[],bool &quedarse);  //lee coordenadas de movimiento, verifica y lo despliga, a�adiendo puntos respectivos  //obtener la inicial de un nombre
    ...
    do
    {
        ...
        if (opt==1)
        {
            do
            {
                   ...
                   for(int a=0; a<num_jugadores; a++)
                    {
                        ...
                        else if(a==num_jugadores-1)
                        {
                            cout<<"Jugador no hallado"<<endl;
                                ...
                                cout<<"2)presione cualquier otro n�mero para cambiar de gamertag"<<endl;
                                cin>>opt;
                                if(opt==1)
								{
                ...
                {
                    num_jugadores=vectorizar(jugadores);
                    cout<<"Digite el n�mero de jugadores de esta partida: ";
                    cin>> num;
                    for(int i=0;i<num; i++)
                    {
                    ...
                    for(int i=0;i<num;i++)  //ciclo de obtenci�n de iniciales
                    {
                    ...
                    while(faltan(tablero,filas, columnas) && (quedarse == true)) //ciclo de juego
                    {
                        ...
                        borrar();
                        actualizar(filas, columnas,tablero);
                        turno++;  //gu�a para contadores ,vectores y cuadros completados
                ...
                else if (opt==3)
                {
                    ...
                    cout << "3) Los movimientos se realizan por posiciones en x1, y1, x2, y2 cuyo origen est� en la esquina superior izuquierda."<<endl;
                    cout << "4) Los movimientos son v�lidos si y solo si no se superponen a otros, son continuos y en espacios v�lidos. "<<endl;
                    break;
                }
            }while (opt!=4);
        }
        ...
        else if (opt==4)
        {
            ...
            case 4:
                ...
                break;
            default:
                cout<<"Redireccionando al men�"<<endl;
            }
...
{
    for(int k=0; k<fila; k++)  //llenado y actualizaci�n de la matriz
    {
        for(int l=0; l<col;l++)
        {
            cout<<tablero[k][l]<<" ";
...
bool valido(int x1,int x2,int y1, int y2)
{
    ...
    cf=pow((x2-x1),2);
    cc=pow((y2-y1),2);
    if(cf > 1 || cc > 1)  //si los coordenadas suman m�s para ser continuas, se descartan
    {
        return false;
    }
...
{
    if(tablero[x][y]==' ') //verifica si el espacio est� libre
    {
        return true;
    }
    return false;
...
bool completo(int x,int y,int fila, int col,char tablero[][M_columnas],int &pos)
{
    ...
    return false;  //si no ha completado ning�n cuadrodo no manda nada
}
...
bool faltan(char tablero[][M_columnas],int fila, int col)
{
   for(int c=1;c<fila;c=c+2)
   {
       for(int d=1;d<col;d=d+2)
       {
           if(tablero[c][d]==' ')
           {
               return true; // si a�n hay espacios continua el juego
           }
       }
   }
...
void asig_puntos(jugador jugadores[], jugador partida[],int num, int puntos[], int num_jugadores)
{
    for (int in=0; in<num; in++)
    {
        ...
        if(strcmp(partida[in].gamertag,jugadores[j].gamertag)==0)
            ...
            {
                cout<<"Ha ocurrido un problema en la asignaci�n de datos"<<endl;
            }
        }
    }
    cout<< "Asignaci�n completa, gracias por jugar"<<endl;
    return;
}
void ganador(int puntos [], int num,jugador jugadores[], int turnos, int num_jugadores, jugador partida[])
{
    ...
    for (int a=0;a<num_jugadores;a++)
    {
        ...
        {
          jugadores[a].partidas_ganadas++;
		  cout<<"Ganador: "<<partida[puntero].nom_jugador<<" o "<<partida[puntero].gamertag<<endl;
...
void tu_turno(int fila, int col,int &turno,char tablero[][M_columnas],jugador partida[],int puntos[],bool &quedarse)
{
    ...
    cin>> y2;
    cout<<endl;
    if(x1 <=0 || x2<=0 || y1<=0 || y2<=0)  //condici�n de salida del juego
    {
        quedarse = false;
    }
    else
    {
        x= x1+x2-2;  //asignaci�n para coordenada de matriz tablero
        y= y1+y2-2;
        while(!(valido(x1,x2,y1,y2)) || !(libre(x,y,tablero)))  //verificaci�n de coordenadas, si son v�lidas
        {
            cout<<"Movimiento inv�lido, vuelva a digitar su movimiento por favor: "<<endl;
            cout<<"Digite la coordena x1: ";
            cin>> x1;
            cout<<"Digite la coordena y1: ";
            cin>> y1;
        ...
        if(x%2 == 0 && y%2 !=0)  //asignaci�n de movimiento de acuerdo a posicion en la matriz
        {
            tablero[x][y]= '-';
        }
        else
        ...
        if(gan==true)
        {
            ...
            turno--; //para repetici�n de turno
        }
    }
    return;
}
...
//_______________________________________EXPORTACI�N Y ARCHIVOS__________________________________________
void creacion_usuario(jugador gamer)
{
    jugador aux;
    string nom;
    ...
    if(contador <= 20)
    {
           cout<<"Jugador no existente, se proceder� a la creaci�n del nuevo jugador:"<<endl;
            aux.codigo = cod;
            cod++;
...
void borrado_usuario(jugador gamer, string gamertag)
{
    ...
    else
    {
        ...
        if(comprobar)
        {
            cout<<"Eliminaci�n correctamente realizada"<<endl;
        }
        else
        {
...
void desv(jugador jugadores[], int num_jugadores)
{
    int liar = 0;
	jugador gamer;
    archivou = fopen("temporal.txt","w");
    while(true)
    {
    	fwrite(&jugadores[liar], sizeof(jugadores[liar]),1,archivou);
    	liar++;
		if(liar >=num_jugadores)
		{
			break;
		}
	}
	fclose(archivou);
	remove("lista_jugadores.txt");
	rename("temporal.txt","lista_jugadores.txt");
    return;
}
void inicial(string nombres,char &inicial)
{
